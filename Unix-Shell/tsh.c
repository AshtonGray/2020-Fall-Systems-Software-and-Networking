...
void eval(char *cmdline) 
{
	...
	// ------------------------------------------------------
	    ...
	    // ------ BackGround ------
            ...
            sigprocmask(SIG_UNBLOCK, &mask, &prev_mask);     // Unblocks SIGCHLD signal
...
void sigtstp_handler(int sig) 
{
	...
	if (verbose == 1)
		printf("entering sigstp_handler\n");
	...
	if (pid != 0) // if foreground process exists
	{
		kill(-pid, sig); // kills all foreground processes in a group
		if (verbose == 1)
			printf("sigstp_handler: Job [%d] and its entire foreground jobs with same process group are killed\n", (int)pid);
	}

	if (verbose == 1)
		printf("exiting sigstp_handler\n");
