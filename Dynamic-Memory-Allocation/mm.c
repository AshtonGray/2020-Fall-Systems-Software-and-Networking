...
void* mm_malloc (size_t size) {
  ...
  // examine_heap();
	...
	ptrFreeBlock = searchFreeList(size); // returns pointer to block
	if (ptrFreeBlock == NULL){ // if there is no more space
		requestMoreSpace(size); // add space
		ptrFreeBlock = searchFreeList(size); // returns pointer to block
	...
	// need to see if the tag already has a preceding block
  ...
  if(blockSize >= MIN_BLOCK_SIZE + reqSize) {   
    ...
    // Split a large block to two pieces 
	...
	ptrSecondBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);

    // Save the new size and tag info in the block info of the next block. 
    // Tag it to show the preceding block is used 
	ptrSecondBlock->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
	
    // Setting the boundary tag for the 2nd block

	((BlockInfo*)UNSCALED_POINTER_ADD(ptrSecondBlock, blockSize-reqSize-WORD_SIZE))->sizeAndTags = (blockSize-reqSize) | TAG_PRECEDING_USED;

    // Put the new block in the free list using both insertFreeBlock() and coalesceFreeBlock()
	insertFreeBlock(ptrSecondBlock);
	coalesceFreeBlock(ptrSecondBlock);
	...
	ptrNextBlock->sizeAndTags |= TAG_PRECEDING_USED; // orr the size of the block (64 bit binary with 3 lsb's = 0) with tag used (bit 0 = 1 : block is used)
	
  }
	ptrFreeBlock->sizeAndTags |= precedingBlockUseTag | TAG_USED;
...
void mm_free (void *ptr) {
  ...
  *((size_t*) UNSCALED_POINTER_ADD(blockInfo, payloadSize)) = blockInfo->sizeAndTags;
  ...
  // Get the next block and update its header (since its preceding blick is now free)
